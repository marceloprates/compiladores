...
/*char* ASTtoString(AST* ast)
{
	...
	else
	{
		...
		{
			childrenStrings[i] = ASTtoString(ast->child[i]);
		}

		int fatherStringLength = strlen(NodeTypetoString(ast->node_type));
		...
		switch(ast->numChildren)
		{
			case 0:
			{
				switch(ast->node_type)
				{
					case LITERAL:
					{
						switch(ast->node->symbol.type)
						{
							case SYMBOL_LIT_INTEGER:
								sprintf(fatherString,"(%s %d)",NodeTypetoString(ast->node_type), ast->node->symbol.value.intLit);
								break;
							case SYMBOL_LIT_TRUE:
								sprintf(fatherString,"(%s TRUE)",NodeTypetoString(ast->node_type));
								break;
							case SYMBOL_LIT_FALSE:
								sprintf(fatherString,"(%s FALSE)",NodeTypetoString(ast->node_type));
								break;
							case SYMBOL_LIT_CHAR:
								sprintf(fatherString,"(%s %c)",NodeTypetoString(ast->node_type), ast->node->symbol.value.charLit);
								break;
							case SYMBOL_LIT_STRING:
								sprintf(fatherString,"(%s %s)",NodeTypetoString(ast->node_type), ast->node->symbol.value.stringLit);
								break;
						}
						break;
					}
					case IDENTIFIER:
					{
						sprintf(fatherString,"(%s %s)",NodeTypetoString(ast->node_type), ast->node->symbol.value.identifier);
						break;
					}
				}
				break;
			...
			case 1:
				sprintf(fatherString,"(%s %s)",NodeTypetoString(ast->node_type),childrenStrings[0]);
				break;
			case 2:
				sprintf(fatherString,"(%s %s %s)",NodeTypetoString(ast->node_type),childrenStrings[0],childrenStrings[1]);
				break;
			case 3:
				sprintf(fatherString,"(%s %s %s %s)",NodeTypetoString(ast->node_type),childrenStrings[0],childrenStrings[1],childrenStrings[2]);
				break;
			case 4:
				sprintf(fatherString,"(%s %s %s %s %s)",NodeTypetoString(ast->node_type),childrenStrings[0],childrenStrings[1],childrenStrings[2],childrenStrings[3]);
				break;
		}
